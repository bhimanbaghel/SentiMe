...
class NaiveBayesClassifier:
    ...
    def __init__(self, data, keyword, time, trainingDataFile, classifierDumpFile,trainingRequired=0):
        ...
        self.helper = classifier_helper.ClassifierHelper('./scripts/data/feature_list.txt')
    ...
    def getFilteredTrainingData(self, trainingDataFile):
        fp = open( trainingDataFile, 'r',encoding='latin-1')
    ...
    def classify(self):        
        ...
        pyplot.show()'''
        posSent = "%.2f" % (self.pos_count/count*100.0)
        negSent = "%.2f" % (self.neg_count/count*100.0)
        neutSent = "%.2f" % (self.neut_count/count*100.0)
        return { "positive_sentiment": posSent, "negative_sentiment": negSent, "neutral_sentiment": neutSent,}
    #end
